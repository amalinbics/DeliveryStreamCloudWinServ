    ...
    public class WebServices : ServiceBases
    {
        ...
        public static void CallWebService()
        {
            try
            {
                ...
                try
                {
                    Logging.WriteToFile1(string.Format("UpdateRejectedLoads is called at {0}", DateTime.Now));
                    UpdateRejectedLoads();
                    Logging.WriteToFile1(string.Format("UpdateRejectedLoads is completed at {0}", DateTime.Now));
                }
                catch (Exception ex)
                {
                    Logging.LogError(ex);
                ...
                try
                {
                    Logging.WriteToFile1(string.Format("UpdateDeliveryNotes is called"));
                    UpdateDeliveryNotes();
                    Logging.WriteToFile1(string.Format("UpdateDeliveryNotes is completed"));
                }
                catch (Exception ex)
                {
                    Logging.LogError(ex);
        ...
        public static void UpdateLoad()
        {
            ...
            try
            {
                ...
                List<DAL.LoadRow> rows = DALMethods.GetUpdatedLoad(session, VersionNo);
                if (rows != null && rows.Count > 0)
                {
                    rows.Sort((x, y) => string.Compare(x.CustomerID_Ext, y.CustomerID_Ext));
                    foreach (DAL.LoadRow row in rows)
                    {
                        try
                        {
                            ...
                            else
                            {
                                ...
                                client.UpdateLoad(cust.CustomerID_Ext, cust.Password_Ext, row.LoadNo_Ext, row.VehicleID_Ext, row.DriverID_Ext, VersionNo);
                                DALMethods.UpdateLoad(row.Id_Ext, session, VersionNo);
                            }
                        ...
                        {
                            Logging.WriteLog(String.Format("Error for Customer customer {0} and LoadID {1}.", row.CustomerID_Ext, row.Id_Ext), EventLogEntryType.Error);
                            Logging.LogError(ex);
                        }
                    }
                }
        ...
        public static void UpdateLoadStatus()
        {
            ...
            try
            {
                ...
                List<DAL.LoadStatusHistoryRow> rows = DALMethods.GetUpdatedLoadStatusHistory(session, VersionNo);
                List<DAL.LoadRow> lstLoad = DALMethods.GetUndispatchedLoad(session, VersionNo);
                    ...
                    foreach (DAL.LoadRow load in lstLoad)
                    {
                        try
                        {
                            ...
                            else
                            {
                                ...
                                client.UpdateLoadStatus(cust.CustomerID_Ext, cust.Password_Ext, load.LoadNo_Ext, status, VersionNo);
                ...
                if (rows != null && rows.Count > 0)
                {
                    ...
                    foreach (DAL.LoadStatusHistoryRow row in rows)
                    {
                        try
                        {
                            ...
                            {
                                CustomerServiceClient client = GetServiceClient(cust.WCFURL_Ext, VersionNo);
                                client.UpdateLoadStatus(cust.CustomerID_Ext, cust.Password_Ext, row.LoadNo_Ext, row.LoadStatusID_Ext, VersionNo);

                                //On Sucess
                                DALMethods.UpdateLoadStatusHistory(row.LoadID_Ext, row.LoadStatusID_Ext, session, VersionNo);

                                string IsEnabledFrtBrk = DALMethods.IsEnabledFrkBrkdown(cust.CustomerID_Ext, session, VersionNo);
                                if (IsEnabledFrtBrk.ToUpper().Trim() == "TRUE")
                                {
                                    if (row.LoadStatusID_Ext.ToUpper().Trim() == "L" || row.LoadStatusID_Ext.ToUpper().Trim() == "U")
                                    {
                                        List<DAL.Cloud_GetMilesByLoadRow> lstGetMilesByLoad = DALMethods.GetFreightBreakdownDetails(cust.CustomerID_Ext, row.LoadNo_Ext, session, VersionNo);
                                        foreach (DAL.Cloud_GetMilesByLoadRow LRow in lstGetMilesByLoad)
                                        {
                                            client.UpdateFreightBreakdown(cust.CustomerID_Ext, cust.Password_Ext, LRow.SysTrxNo, 0, Convert.ToChar(LRow.Status), LRow.OriginCity, LRow.DestCity, LRow.OriginState, LRow.DestState, 0, LRow.TotalMiles, Convert.ToDecimal(LRow.OriginLat), Convert.ToDecimal(LRow.OriginLong), Convert.ToDecimal(LRow.DestLat), Convert.ToDecimal(LRow.DestLong), VersionNo);
                                        }
                                    }
                                }
        ...
        public static void UpdateOrderStatus()
        {
            ...
            {
                session = GetNewSession();
                List<DAL.OrderStatusHistoryRow> rows = DALMethods.GetUpdatedOrderStatusHistory(session, VersionNo);
                if (rows != null && rows.Count > 0)
                {
                    //rows.Sort((x, y) => string.Compare(x.CustomerID_Ext, y.CustomerID_Ext));
                    foreach (DAL.OrderStatusHistoryRow row in rows)
                    {
                        try
                        {
                            ...
                            {
                                CustomerServiceClient client = GetServiceClient(cust.WCFURL_Ext, VersionNo);
                                client.UpdateOrderStatus(cust.CustomerID_Ext, cust.Password_Ext, row.SysTrxNo_Ext, row.OrderStatusID_Ext, VersionNo);
        ...
        public static void UpdateShipmentDetails()
        {
            ...
            {
                session = GetNewSession();
                DataTable dtSD = DALMethods.GetShipmentData1(ShipmentRetryCount, session, VersionNo);
                                ...
                                if (bolItems.Rows.Count > 0)
                                {
                                    ...
                                    for (var j = 0; j < bolItems.Rows.Count; j++)
                                    {
                                        if (DALMethods.IsSplitLoad(new Guid(bolItems.Rows[j]["LoadID"].ToString()), new Guid(), 1, session))
                                        {
                                            ...
                                            {
                                                client.UndoShipDoc(cust.CustomerID_Ext, cust.Password_Ext, Convert.ToInt32(bolItems.Rows[j]["ShipDocSysTrxNo"].ToString()), 'A', VersionNo);
                                            }
                                        }
                                        ShipmentRequestComponents component = new ShipmentRequestComponents();
                                    ...
                                    if (cust.CustomerID_Ext == ConfigurationManager.AppSettings["BOLWaitTimeDetails"])
                                    {
                                        ...
                                        if (dtBD != null && dtBD.Rows.Count > 0)
                                        {
                                            ...
                                            {
                                                client.UpdateBOLWaitTimeDetails(cust.CustomerID_Ext, cust.Password_Ext, cust.CustomerID_Ext, Convert.ToDecimal(dtBD.Rows[k]["SysTrxNo"]), dtBD.Rows[k]["BOLNo"].ToString(), Convert.ToDateTime(dtBD.Rows[k]["BOLWaitTimeStart"].ToString()), Convert.ToDateTime(dtBD.Rows[k]["BOLWaitTimeEnd"].ToString()), dtBD.Rows[k]["BOLWaitTimeComment"].ToString(), "");
                                            }
                                        }
                                    }
        ...
        public static void UpdateDeliveryDetails()
        {
            ...
            try
            {
                List<Guid> OrderItemIDs = new List<Guid>();
                session = GetNewSession();
                DataTable rows = DALMethods.GetUpdatedDeliveryOrders1(DeliveryRetryCount, session, VersionNo);                
                    ...
                    for (var i = 0; i < rows.Rows.Count; i++)
                    {
                        try
                        {
                            ...
                            else
                            {
                                ...
                                Logging.WriteToFile1(cust.CustomerID_Ext + " - " + cust.Password_Ext + " - " + req + " - " + VersionNo);
        ...
        /// </summary>
        public static void UpdateBOLImages()
        {
            ISession session = null;
            try
            {
                session = GetNewSession();
                DataTable dtSD = DALMethods.GetDataToUpdateBOLImage(session, VersionNo);
                if (dtSD != null && dtSD.Rows.Count > 0)
                {
                    for (var i = 0; i < dtSD.Rows.Count; i++)
                    {
                        try
                        {
                            ...
                            else
                            {
                                ...
                                client.UpdateBOLImage(cust.CustomerID_Ext, cust.Password_Ext, image,imagepdf, Systrxno, dtSD.Rows[i]["BOLNo"].ToString(), VersionNo);
        ...
        public static void SendEmailforRejectedLoads()
        {
            ISession session = null;
            try
            {
                session = GetNewSession();
                DataTable dtRejectedLoads = DALMethods.GetRejectedLoads(session, VersionNo);
        ...
        /// </summary>
        public static void UpdatePONo()
        {
            ISession session = null;
            try
            {
                session = GetNewSession();
                DataTable dtSD = DALMethods.GetDataToUpdatePONo(session, VersionNo);
                if (dtSD != null && dtSD.Rows.Count > 0)
                {
                    for (var i = 0; i < dtSD.Rows.Count; i++)
                    {
                        try
                        {
                            ...
                            else
                            {
                                ...
                                client.UpdateOrderPONo(cust.CustomerID_Ext, cust.Password_Ext, OrderNo, Systrxno, PONo, VersionNo);
        ...
        public static void UpdateRejectedLoads()
        {
            ...
            try
            {
                ...
                DataTable dtRejectedLoads = DALMethods.FetchRejectedLoads(session);
        ...
        public static void UpdateDeliveryNotes()
        {
            ISession session = GetNewSession();
            ...
            try
            {
                ...
                DataTable dtDeliveryNotes = DALMethods.FetchDeliveryNotes(session);
